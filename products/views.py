...
def all_products(request):
    """
    A view to show all products, also sorting and search queries
    """
    ...
    if request.GET:
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
                messages.error(request, "You have not searched for any product.")  # noqa
...
def delete_product(request, product_id):
    ...
    product = get_object_or_404(Product, pk=product_id)
    product.delete()
    messages.success(request, f'Product {product.name} was deleted successfully!')  # noqa
    return redirect(reverse('products'))
